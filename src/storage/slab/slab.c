...
static void
_slab_restore_item_to_freeq(struct item *it, uint8_t id)
{
...
static void _slab_lruq_append(struct slab *slab);

static void
_slab_iterate_over_all_items(struct slab *slab)
{
    struct slabclass *p;
    struct item *it;
    uint32_t i;
    ...
    for (i = 0; i < p->nitem; i++) {
        ...
        if (it->is_linked) {
            ...
            _slab_restore_item_to_freeq(it,slab->id);
        }
    }
...
_slab_table_update(struct slab *slab)
{
    ...
    log_crit("new slab %p allocated at pos %u", slab,
              heapinfo.nslab - 1);
}

struct slab_pmem_metadata {
    ptrdiff_t saved_pool_adress;             /* saved pool adresss */
    void* heap_info_slab_lruq_address;       /* lru slab q head*/
};
...
_slab_recreate (void)
{
    ...
    struct slab_pmem_metadata * temp_user_data = datapool_get_user_data(pool_slab, sizeof (struct slab_pmem_metadata));
    ptrdiff_t offset = (ptrdiff_t)(uint8_t*)datapool_addr(pool_slab) - (ptrdiff_t)temp_user_data->saved_pool_adress;
    TAILQ_FIRST(&heapinfo.slab_lruq) = (struct slab *) temp_user_data->heap_info_slab_lruq_address;
    cc_free(temp_user_data);
    uint32_t slabno = 0;
    for(i = 0; i < heapinfo.max_nslab; i++) {
        ...
        if (slab_iterator->magic == SLAB_MAGIC) {
            ...
            uint8_t * tqe_next = (uint8_t*) slab_iterator->s_tqe.tqe_next;
            uint8_t * tqe_prev = (uint8_t*) slab_iterator->s_tqe.tqe_prev;
            tqe_next += offset;
            tqe_prev += offset;
            slab_iterator->s_tqe.tqe_next = ( struct slab *) tqe_next;
            slab_iterator->s_tqe.tqe_prev = ( struct slab *) tqe_prev;
    ...
    uint8_t * heap_calculate_second = datapool_addr(pool_slab);
    for(i = 0; i < slabno; i++) {
        struct slab *slab = (struct slab *) heap_calculate_second;
        _slab_lruq_remove(slab);
         heap_calculate_second += slab_size;
    }
...
{
    struct slab_pmem_metadata test = {
    (ptrdiff_t)(uint8_t *)datapool_addr(pool_slab),
    TAILQ_FIRST(&heapinfo.slab_lruq)};
    log_crit("_slab_heapinfo_teardown heapinfo.slab_lruq.tqh_first %p", TAILQ_FIRST(&heapinfo.slab_lruq));
    if (!datapool_set_user_data(pool_slab, &test, sizeof(struct slab_pmem_metadata))) {
        log_crit("heapinfo slab lruq %p", &heapinfo.slab_lruq);
    }
...
{
    ASSERT(id >= SLABCLASS_MIN_ID && id <= profile_last_id);
#if defined CC_ASSERT_PANIC || defined CC_ASSERT_LOG || USE_PMEM_VARIANT
 ...
 *
 * Eviction complexity is O(#items/slab).pool_slab_state
 */
static void
