...
_slab_put_item_into_freeq(struct item *it, uint8_t id)
{
    ...
    loga("put free q it %p at offset %"PRIu32" with id %"PRIu8, it,
            it->offset, it->id);
...
static void _slab_init_lru(struct slab * slab, ptrdiff_t offset)
{
...
static void _slab_update_lru(struct slab * slab, ptrdiff_t offset)
{
...
_slab_get_item(uint8_t id)
{
    ...
    loga("get new it at offset %"PRIu32" with id %"PRIu8"",
              it->offset, it->id);
