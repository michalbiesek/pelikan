...
static void
_slab_heapinfo_base_prealloc_setup()
{
    if (slab_datapool) {
        pool_slab = datapool_open(slab_datapool, slab_mem, &fresh_datapool);
        if (pool_slab == NULL) {
            log_crit("Could not setup pool_slab");
            exit(EX_CONFIG);
        }
    }

    if (fresh_datapool) {
        heapinfo.base = datapool_extent(pool_slab, heapinfo.max_nslab * slab_size);
    }
    else {
...
_slab_heapinfo_setup(void)
{
    ...
    if (prealloc) {
        ...
        /* TODO(mbiesek): add implementation for PMEM regarding !prealloc */
        log_crit("PMEM is supporting only prealloc");
        return CC_EINVAL;
    }
